...
// Four match_type shapes:
// - sale_payment / vendor_payment: links an already-existing row (sale_payment_id /
//   vendor_payment_id must already exist -- this route never creates those).
...
export async function POST(
  ...
  const { match_type, sale_payment_id, vendor_payment_id, expense, counterpart_txn_id, amount_applied, variance_reason } = body
  ...
  if (match_type === 'sale_payment') {
    ...
    if (!vendor_payment_id) return NextResponse.json({ error: 'vendor_payment_id is required.' }, { status: 400 })
    ...
    return NextResponse.json({ error: 'match_type must be sale_payment, vendor_payment, expense, or transfer_pair.' }, { status: 400 })
  }
  ...
  const { data: match, error: matchErr } = await supabaseAdmin
    ...
    .insert({
      ...
      vendor_payment_id: match_type === 'vendor_payment' ? vendor_payment_id : null,
