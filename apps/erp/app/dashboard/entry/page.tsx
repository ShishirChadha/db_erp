...
function EntryLauncherPage() {
  return (
    <div className="p-4 max-w-3xl mx-auto">
      ...
      <p className="text-sm text-muted-foreground mb-6">What are you recording?</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {TILES.map((tile) => {
          ...
          const content = (
            <div
              className={`flex flex-col items-center text-center gap-2 p-6 rounded-xl border bg-card shadow-sm transition-all ${
                tile.enabled ? 'hover:shadow-md hover:border-primary/30 cursor-pointer' : 'opacity-50 cursor-not-allowed'
              }`}
            >
              ...
              {!tile.enabled && <span className="text-xs text-muted-foreground mt-1">Coming soon</span>}
            </div>
          )
          return tile.enabled ? (
