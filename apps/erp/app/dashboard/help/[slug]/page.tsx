...
export default function DbGuideChapterPage() {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    apiFetch(`/api/db-guide/${encodeURIComponent(slug)}`)
      .then(async (res) => {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <Link href="/dashboard/help" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-4">
        <ChevronLeft className="h-4 w-4" /> Back to DB Guide
      </Link>
