    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded shadow-lg max-w-md w-full space-y-3">
          ...
          <ul className="text-sm border rounded divide-y">
            {attachmentUrls.map((key, i) => (
              <li key={key} className="p-2 flex items-center justify-between gap-2">
                ...
                <button onClick={() => viewFile(key)} className="text-blue-600 underline text-xs inline-flex items-center gap-1 shrink-0">
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button onClick={() => save()} disabled={busy || !file} className="px-4 py-2 bg-blue-600 text-white rounded text-sm inline-flex items-center gap-1.5 disabled:opacity-50">
