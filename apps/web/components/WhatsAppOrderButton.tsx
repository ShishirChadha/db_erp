// Plain server-renderable link (like WhatsAppButton.tsx) -- no client component
// needed for a wa.me href. `href` is built by the caller (product page has the
// title/url/price already computed) so this component owns styling only, not
...
export function WhatsAppOrderButton({
  ...
  href: string
  className?: string
  /** Icon-only square button, for tight spaces like StickyBuyBar. */
  compact?: boolean
}) {
  return (
    <a
      ...
      className={
        compact
          ? `flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#25D366] text-white transition-colors hover:bg-[#1ebe5a] ${className}`
          : `flex w-full items-center justify-center gap-2 rounded-lg bg-[#25D366] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#1ebe5a] ${className}`
      }
    >
