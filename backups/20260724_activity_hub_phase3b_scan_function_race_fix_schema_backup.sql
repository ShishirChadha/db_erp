...
BEGIN
  FOR act IN
    SELECT a.id, a.title, a.created_by, a.due_date
    FROM activities a
  ...
  LOOP
    ...
    END LOOP;
    UPDATE activities SET due_soon_notified_at = now() WHERE id = act.id;
  END LOOP;
  ...
  LOOP
    ...
    LOOP
      ...
      VALUES (recipient, 'overdue', NULL, act.id, act.title,
              ...
              '/dashboard/activities?open=' || act.id);
    END LOOP;
    UPDATE activities SET overdue_notified_at = now() WHERE id = act.id;
  END LOOP;
END;
$$;
