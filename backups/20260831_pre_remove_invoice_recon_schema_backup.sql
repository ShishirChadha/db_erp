...
CREATE OR REPLACE FUNCTION "public"."match_skus_for_invoice_line"("p_text" "text", "p_limit" integer DEFAULT 8) RETURNS TABLE("sku_id" "uuid", "full_sku_code" "text", "sku_description" "text", "brand" "text", "model_name" "text", "category" "text", "trgm_score" real, "is_serialized" boolean)
    LANGUAGE "sql" STABLE SECURITY DEFINER
    SET "search_path" TO 'public', 'extensions'
    AS $$
  with p_all_tokens as (
    select array_agg(distinct t) as tokens
    from regexp_split_to_table(lower(p_text), '[^a-z0-9]+') t
    where t <> ''
  ),
  -- Digit runs anywhere in the raw text, not just standalone whitespace-delimited
  -- tokens -- a real invoice line often fuses the model number onto the previous
  -- word with no space ("Probook440"), which a token-split approach misses
  -- entirely (it reads as one alnum token, never a pure-digit one) and would then
  -- wrongly exclude the correct SKU as a "different model number".
  p_num_tokens as (
    select array_agg(m[1]) as tokens
    from regexp_matches(p_text, '[0-9]{2,}', 'g') as m
  )
  select s.id, s.full_sku_code, s.sku_description, s.brand, s.model_name, s.category,
         greatest(
           similarity(coalesce(s.sku_description, ''), p_text),
           similarity(coalesce(s.brand, '') || ' ' || coalesce(s.model_name, ''), p_text)
         ) as trgm_score,
         exists (select 1 from asset_ledger al where al.sku_id = s.id and al.po_id is null and al.is_deleted = false) as is_serialized
  from sku_master s, p_all_tokens pat, p_num_tokens pnt
  left join lateral (
    select category
    from (values
      ('\mram\M|\mmemory\M', 'RAM', 1),
      ('\mssd\M|\mnvme\M|\mhdd\M|\mhard\s*disk\M', 'SSD', 1),
      ('\mkeyboard\M', 'KBD', 1),
      ('\mmouse\M', 'MOUSE', 1),
      ('\madapter\M|\mcharger\M', 'ADP', 1),
      ('\mcamera\M|\mcctv\M', 'CCTV', 1),
      ('\mgraphics\s*card\M|\mgpu\M', 'GPU', 1),
      ('\mprocessor\M', 'CPU', 1),
      ('\mbattery\M|\mheadset\M|\mwebcam\M|\mhinge\M|\mbezel\M|\mpalmrest\M|\mtouchpad\M|\mtrackpad\M|\mdisplay\s*panel\M', 'ACC', 1),
      ('\mlaptop\M|\mnotebook\M', 'LAP', 2),
      ('\mdesktop\M|\mcpu\s*cabinet\M|\mtower\M|\mall[- ]?in[- ]?one\M', 'DES', 2),
      ('\mtablet\M|\mipad\M', 'TAB', 2),
      ('\mmonitor\M', 'MON', 2)
    ) as kw(pattern, category, priority)
  ...
  where s.status != 'archived'
    and (
      exists (select 1 from asset_ledger al where al.sku_id = s.id and al.po_id is null and al.is_deleted = false)
      or exists (select 1 from stock_movements sm where sm.sku_id = s.id and sm.movement_type = 'receipt' and sm.po_id is null)
    )
    and greatest(
      word_similarity(coalesce(s.sku_description, ''), p_text),
      word_similarity(coalesce(s.brand, '') || ' ' || coalesce(s.model_name, ''), p_text)
    ) > 0.2
    and (
      s.brand is null or s.brand = ''
      or pat.tokens is null
      or lower(s.brand) = any(pat.tokens)
    )
    and (
      pnt.tokens is null
      or not exists (
        select 1 from regexp_matches(coalesce(s.model_name, ''), '[0-9]{3,}', 'g') mt
      )
      or exists (
        select 1 from regexp_matches(coalesce(s.model_name, ''), '[0-9]{3,}', 'g') mt
        where mt[1] = any(pnt.tokens)
      )
    )
    and (pec.category is null or s.category = pec.category)
  order by trgm_score desc
  limit p_limit;
$$;


ALTER FUNCTION "public"."match_skus_for_invoice_line"("p_text" "text", "p_limit" integer) OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."invoice_match_candidates" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
ALTER TABLE "public"."invoice_match_candidates" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."invoice_match_lines" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "invoice_match_lines_match_status_check" CHECK (("match_status" = ANY (ARRAY['unmatched'::"text", 'matched'::"text", 'skipped'::"text"])))
);


ALTER TABLE "public"."invoice_match_lines" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."invoice_match_sessions" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "document_id" "uuid" NOT NULL,
    "vendor_id" "uuid",
    "invoice_number" "text",
    ...
    "grand_total" numeric,
    "status" "text" DEFAULT 'draft'::"text" NOT NULL,
    "committed_po_id" "uuid",
    "committed_invoice_id" "uuid",
    "secondary_po_id" "uuid",
    "created_by" "uuid",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "invoice_match_sessions_status_check" CHECK (("status" = ANY (ARRAY['draft'::"text", 'reviewing'::"text", 'committed'::"text", 'abandoned'::"text"])))
);


ALTER TABLE "public"."invoice_match_sessions" OWNER TO "postgres";
...
ALTER TABLE ONLY "public"."invoice_match_candidates"
    ADD CONSTRAINT "invoice_match_candidates_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."invoice_match_lines"
    ADD CONSTRAINT "invoice_match_lines_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."invoice_match_sessions"
    ADD CONSTRAINT "invoice_match_sessions_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_invoice_match_candidates_line" ON "public"."invoice_match_candidates" USING "btree" ("line_id");



CREATE INDEX "idx_invoice_match_lines_session" ON "public"."invoice_match_lines" USING "btree" ("session_id");



CREATE INDEX "idx_invoice_match_sessions_document" ON "public"."invoice_match_sessions" USING "btree" ("document_id");
...
ALTER TABLE ONLY "public"."invoice_match_candidates"
    ADD CONSTRAINT "invoice_match_candidates_asset_ledger_id_fkey" FOREIGN KEY ("asset_ledger_id") REFERENCES "public"."asset_ledger"("id");



ALTER TABLE ONLY "public"."invoice_match_candidates"
    ADD CONSTRAINT "invoice_match_candidates_line_id_fkey" FOREIGN KEY ("line_id") REFERENCES "public"."invoice_match_lines"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."invoice_match_candidates"
    ADD CONSTRAINT "invoice_match_candidates_sku_id_fkey" FOREIGN KEY ("sku_id") REFERENCES "public"."sku_master"("id");



ALTER TABLE ONLY "public"."invoice_match_lines"
    ADD CONSTRAINT "invoice_match_lines_matched_sku_id_fkey" FOREIGN KEY ("matched_sku_id") REFERENCES "public"."sku_master"("id");



ALTER TABLE ONLY "public"."invoice_match_lines"
    ADD CONSTRAINT "invoice_match_lines_session_id_fkey" FOREIGN KEY ("session_id") REFERENCES "public"."invoice_match_sessions"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."invoice_match_sessions"
    ADD CONSTRAINT "invoice_match_sessions_committed_invoice_id_fkey" FOREIGN KEY ("committed_invoice_id") REFERENCES "public"."invoices"("id");



ALTER TABLE ONLY "public"."invoice_match_sessions"
    ADD CONSTRAINT "invoice_match_sessions_committed_po_id_fkey" FOREIGN KEY ("committed_po_id") REFERENCES "public"."purchase_orders"("id");



ALTER TABLE ONLY "public"."invoice_match_sessions"
    ADD CONSTRAINT "invoice_match_sessions_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "public"."invoice_match_sessions"
    ADD CONSTRAINT "invoice_match_sessions_document_id_fkey" FOREIGN KEY ("document_id") REFERENCES "public"."uploaded_documents"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."invoice_match_sessions"
    ADD CONSTRAINT "invoice_match_sessions_entity_key_fkey" FOREIGN KEY ("entity_key") REFERENCES "public"."business_profiles"("key");



ALTER TABLE ONLY "public"."invoice_match_sessions"
    ADD CONSTRAINT "invoice_match_sessions_secondary_po_id_fkey" FOREIGN KEY ("secondary_po_id") REFERENCES "public"."purchase_orders"("id");



ALTER TABLE ONLY "public"."invoice_match_sessions"
    ADD CONSTRAINT "invoice_match_sessions_vendor_id_fkey" FOREIGN KEY ("vendor_id") REFERENCES "public"."vendors"("id");
...
ALTER TABLE "public"."invoice_match_candidates" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."invoice_match_lines" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."invoice_match_sessions" ENABLE ROW LEVEL SECURITY;
...
REVOKE ALL ON FUNCTION "public"."match_skus_for_invoice_line"("p_text" "text", "p_limit" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."match_skus_for_invoice_line"("p_text" "text", "p_limit" integer) TO "service_role";
...
GRANT ALL ON TABLE "public"."invoice_match_candidates" TO "anon";
GRANT ALL ON TABLE "public"."invoice_match_candidates" TO "authenticated";
GRANT ALL ON TABLE "public"."invoice_match_candidates" TO "service_role";



GRANT ALL ON TABLE "public"."invoice_match_lines" TO "anon";
GRANT ALL ON TABLE "public"."invoice_match_lines" TO "authenticated";
GRANT ALL ON TABLE "public"."invoice_match_lines" TO "service_role";



GRANT ALL ON TABLE "public"."invoice_match_sessions" TO "anon";
GRANT ALL ON TABLE "public"."invoice_match_sessions" TO "authenticated";
GRANT ALL ON TABLE "public"."invoice_match_sessions" TO "service_role";
