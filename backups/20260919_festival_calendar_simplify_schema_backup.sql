...
CREATE TABLE IF NOT EXISTS "public"."festival_calendar" (
    ...
    "theme_key" "text" DEFAULT 'generic'::"text" NOT NULL,
    "greeting_text" "text",
...
COMMENT ON TABLE "public"."festival_calendar" IS 'Owner-editable calendar of festivals for the Marketing Studio''s Festive Post tab -- one row per festival PER YEAR (dates shift yearly, esp. lunar-calendar festivals), not a recurring rule. theme_key selects a matching set of code-drawn vector motifs (see lib/marketing/festival-motifs.tsx); an unrecognized/custom theme_key falls back to a generic celebratory motif.';



COMMENT ON COLUMN "public"."festival_calendar"."theme_key" IS 'One of: diwali, holi, christmas, new_year, tricolor, eid, generic -- generic is the fallback for any festival without a dedicated motif set.';



COMMENT ON COLUMN "public"."festival_calendar"."background_image_path" IS 'Optional owner-uploaded background image (storage path in the product-images bucket, festival-backgrounds/ prefix) composited full-bleed behind the card instead of a plain themed color background.';
