...
CREATE OR REPLACE FUNCTION "public"."generate_rental_agreement_number"() RETURNS "text"
    LANGUAGE "plpgsql"
    SET "search_path" TO 'public'
    AS $$
declare
  ...
  next_num integer;
begin
  insert into public.rental_agreement_counter (year, last_number)
  values (cur_year, 1)
  on conflict (year) do update set last_number = rental_agreement_counter.last_number + 1
  returning last_number into next_num;

  return 'RNT-' || cur_year || '-' || lpad(next_num::text, 3, '0');
end;
$$;


ALTER FUNCTION "public"."generate_rental_agreement_number"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."report_category_entity_matrix"("p_from" "date", "p_to" "date", "p_source" "text", "p_include_financials" boolean DEFAULT false) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  if p_source = 'sales' then
    ...
    from (
      select jsonb_build_object(
        'entity', coalesce(entity, 'Unknown'),
        'category', category_bucket,
        'value', coalesce(sum(revenue_incl), 0),
        'count', coalesce(sum(units), 0)
      ) as row_data
      from (
        select
          entity, revenue_incl, units,
          case
            when line_kind = 'repair' then 'Repair'
            when line_kind = 'rental' then 'Rental'
            when category = 'LAP' then 'Laptops'
            when category = 'DES' then 'Desktops'
            when category is not null then 'Accessories'
            else 'Other'
          end as category_bucket
        from v_report_sale_lines
        where sale_date between p_from and p_to
      ) labeled
      group by entity, category_bucket
    ) t;
    return result;
  end if;

  if p_source = 'purchases' then
    -- Cost-bearing (spend by category/entity) -- same owner-only gate as the
    -- existing vendor breakdown, not a new redaction rule.
    if not p_include_financials then
      return '[]'::jsonb;
    end if;
    select coalesce(jsonb_agg(row_data), '[]'::jsonb) into result
    from (
      select jsonb_build_object(
        'entity', coalesce(purchased_by_type, 'Unknown'),
        'category', category_bucket,
        'value', coalesce(sum(line_total), 0),
        'count', coalesce(sum(quantity), 0)
      ) as row_data
      from (
        select
          purchased_by_type, line_total, quantity,
          case
            when category = 'LAP' then 'Laptops'
            when category = 'DES' then 'Desktops'
            when category is not null then 'Accessories'
      ...
      group by purchased_by_type, category_bucket
    ) t;
    return result;
  end if;
...
ALTER FUNCTION "public"."report_category_entity_matrix"("p_from" "date", "p_to" "date", "p_source" "text", "p_include_financials" boolean) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."report_purchase_kpis"("p_from" "date", "p_to" "date") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  where not coalesce(is_deleted, false)
    and po_date between p_from and p_to;
  ...
  from purchase_orders
  where not coalesce(is_deleted, false)
    and po_date between v_prev_from and v_prev_to;
...
ALTER FUNCTION "public"."report_purchase_kpis"("p_from" "date", "p_to" "date") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."report_rentals"("p_from" "date", "p_to" "date") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  select jsonb_build_object(
    'active_agreements', (select count(*) from rental_agreements
        where status = 'active' and not is_deleted),
    'units_on_rent', (select count(*) from rental_agreement_items i
        join rental_agreements r on r.id = i.agreement_id
        where i.item_status = 'on_rent' and not r.is_deleted),
    'overdue_agreements', (select count(*) from rental_agreements r
        where r.status = 'active' and not r.is_deleted
          and r.expected_return_date is not null
          and r.expected_return_date < current_date
          and exists (select 1 from rental_agreement_items i
                      where i.agreement_id = r.id and i.item_status = 'on_rent')),
    'due_to_bill', (select count(*) from rental_agreements r
        where r.status = 'active' and not r.is_deleted
          and r.next_billing_date is not null
          and r.next_billing_date <= current_date + r.billing_reminder_lead_days),
    -- Deposits are a refundable liability, never revenue -- reported on their own,
    -- deliberately outside every rent/revenue figure below.
    'deposits_held', (select coalesce(sum(coalesce(security_deposit_amount, 0)
                                        - coalesce(deposit_refunded_amount, 0)), 0)
        from rental_agreements
        where status = 'active' and not is_deleted and deposit_received_at is not null),
    'rent_billed_incl', coalesce(sum(revenue_incl), 0),
    'rent_billed_ex_gst', coalesce(sum(revenue_ex_gst), 0),
    'rent_gst', coalesce(sum(gst), 0),
    'rent_collected', coalesce(sum(amount_paid), 0),
    'rent_outstanding', coalesce(sum(outstanding), 0),
    'charge_count', count(*)
  ) into summary
  from v_report_sale_lines
  where line_kind = 'rental' and sale_date between p_from and p_to;

  select coalesce(jsonb_agg(row_data), '[]'::jsonb) into by_customer
  from (
    select jsonb_build_object(
      'customer_name', customer_name_canonical,
      'rent_billed', sum(revenue_incl),
      'rent_collected', sum(amount_paid),
      'outstanding', sum(outstanding),
      'charge_count', count(*)
    ) as row_data, sum(revenue_incl) as sort_key
    from v_report_sale_lines
    where line_kind = 'rental' and sale_date between p_from and p_to
    group by customer_name_canonical
    order by sort_key desc limit 15
  ) t;

  return summary || jsonb_build_object('by_customer', by_customer);
end;
$$;


ALTER FUNCTION "public"."report_rentals"("p_from" "date", "p_to" "date") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."report_web_funnel"("p_from" "date", "p_to" "date", "p_abandon_hours" integer DEFAULT 24) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  select count(*) into checkout_started
    from orders
    where created_at::date between p_from and p_to;
  ...
  select count(distinct ci.customer_id) into abandoned_customers
    from cart_items ci
    where ci.created_at::date between p_from and p_to
      and ci.created_at <= now() - (p_abandon_hours || ' hours')::interval
      and not exists (
  ...
  result := jsonb_build_object(
    ...
    'checkout_to_purchase_rate',
      case when checkout_started > 0 then round(100.0 * purchased / checkout_started, 1) else null end,
    'cart_to_checkout_rate',
      case when cart_customers > 0 then round(100.0 * checkout_started / cart_customers, 1) else null end,
...
ALTER FUNCTION "public"."report_web_funnel"("p_from" "date", "p_to" "date", "p_abandon_hours" integer) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."report_web_funnel_timeseries"("p_from" "date", "p_to" "date") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  carts as (
    select created_at::date as d, count(distinct customer_id) as n
    from cart_items where created_at::date between p_from and p_to
    group by 1
  ),
  starts as (
...
ALTER FUNCTION "public"."report_web_funnel_timeseries"("p_from" "date", "p_to" "date") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."report_website_health"("p_from" timestamp with time zone, "p_to" timestamp with time zone) RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  with base as (
    select * from public.website_health_checks
    where checked_at between p_from and p_to
  ),
  summary as (
    select
      count(*) as total_checks,
      count(*) filter (where ok) as ok_checks,
      case when count(*) > 0 then round(100.0 * count(*) filter (where ok) / count(*), 2) else null end as uptime_pct,
      ...
      (array_agg(ok order by checked_at desc))[1] as last_check_ok
    from base
  ),
  failures as (
    select coalesce(jsonb_agg(jsonb_build_object('checked_at', checked_at, 'status_code', status_code, 'error_message', error_message) order by checked_at desc), '[]'::jsonb) as recent_failures
    from (select * from base where not ok order by checked_at desc limit 20) f
  ),
  hourly as (
    select date_trunc('hour', checked_at) as bucket,
      round(100.0 * count(*) filter (where ok) / count(*), 1) as uptime_pct,
      round(avg(latency_ms) filter (where ok))::int as avg_latency_ms
    from base
    group by 1
  ),
  hourly_json as (
    select coalesce(jsonb_agg(jsonb_build_object('bucket', bucket, 'uptime_pct', uptime_pct, 'avg_latency_ms', avg_latency_ms) order by bucket), '[]'::jsonb) as timeseries
    from hourly
  )
  select jsonb_build_object(
    'total_checks', summary.total_checks,
    'ok_checks', summary.ok_checks,
    'uptime_pct', summary.uptime_pct,
    'avg_latency_ms', summary.avg_latency_ms,
    'p95_latency_ms', summary.p95_latency_ms,
    'last_check_at', summary.last_check_at,
    'last_check_ok', summary.last_check_ok,
    'recent_failures', failures.recent_failures,
    'timeseries', hourly_json.timeseries
  )
  into result
  from summary, failures, hourly_json;
...
ALTER FUNCTION "public"."report_website_health"("p_from" timestamp with time zone, "p_to" timestamp with time zone) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."run_website_health_check"("p_url" "text" DEFAULT 'https://www.digitalbluez.com/api/health'::"text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
  ...
  resp_error_msg text;
  resp_created timestamptz;
  found_response boolean;
  new_req_id bigint;
begin
  for pending in
    select id, request_id, checked_at from public.website_health_checks
    where ok is null and request_id is not null
  loop
    select status_code, timed_out, error_msg, created
      into resp_status_code, resp_timed_out, resp_error_msg, resp_created
      from net._http_response where id = pending.request_id;
    found_response := found;

    if found_response then
      update public.website_health_checks
      set ok = (resp_status_code between 200 and 299) and not coalesce(resp_timed_out, false),
          status_code = resp_status_code,
          latency_ms = greatest(0, extract(milliseconds from resp_created - pending.checked_at)::integer),
          error_message = case when resp_timed_out then 'timed out' else resp_error_msg end
      where id = pending.id;
    elsif pending.checked_at < now() - interval '10 minutes' then
      update public.website_health_checks
      set ok = false, error_message = 'no response received'
      where id = pending.id;
    end if;
  end loop;

  begin
    new_req_id := net.http_get(url := p_url, timeout_milliseconds := 8000);
    insert into public.website_health_checks (url, ok, request_id) values (p_url, null, new_req_id);
  exception when others then
    insert into public.website_health_checks (url, ok, error_message) values (p_url, false, sqlerrm);
  end;
end;
$$;


ALTER FUNCTION "public"."run_website_health_check"("p_url" "text") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."scan_rental_cycles"() RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
  ...
  period_end date;
begin
  -- 1. Billing due (or due within the lead window).
  for r in
    update rental_agreements a
    set billing_notified_at = now(),
        next_billing_date = a.next_billing_date + (case a.billing_interval
              when 'monthly' then interval '1 month'
    ...
    where a.status = 'active'
      ...
      and a.billing_interval <> 'one_time'
      and a.next_billing_date is not null
      and a.next_billing_date <= (current_date + a.billing_reminder_lead_days)
      and (a.billing_notified_at is null
           or a.billing_notified_at < (now() - interval '20 days'))
    returning a.id, a.agreement_number, a.created_by, a.billing_interval,
      (a.next_billing_date - (case a.billing_interval
            when 'monthly' then interval '1 month'
            when 'quarterly' then interval '3 months'
            else interval '0 day'
          end))::date as cycle_start,
      (a.next_billing_date - interval '1 day')::date as cycle_end
  loop
    recipient := r.created_by;
    if recipient is null then
      select id into recipient from profiles where role = 'owner' and is_active limit 1;
    end if;
    continue when recipient is null;

    period_start := r.cycle_start;
    period_end := r.cycle_end;

    insert into activities (user_id, title, description, due_date, related_type, related_id,
                            created_by, priority, status)
    values (
      recipient,
      'Raise rent for ' || r.agreement_number,
      'Billing period ' || to_char(period_start, 'DD Mon YYYY') || ' to ' || to_char(period_end, 'DD Mon YYYY')
        || '. Generate the charge from the rental agreement page.',
      period_start::timestamptz,
      'rental_agreement', r.id, recipient, 'normal', 'pending'
    )
    returning id into act_id;
    ...
    insert into notifications (recipient_id, type, actor_id, activity_id, title, body, link)
    values (recipient, 'task_assigned', null, act_id,
      'Rent due to bill: ' || r.agreement_number,
      'Period ' || to_char(period_start, 'DD Mon') || ' to ' || to_char(period_end, 'DD Mon YYYY'),
      '/dashboard/rentals/' || r.id);
  end loop;

  -- 2. Overdue returns. Overdue itself is never stored -- this marker is dedup only.
  for r in
    update rental_agreements a
    set overdue_notified_at = now()
    where a.status = 'active'
      ...
      and a.overdue_notified_at is null
      and exists (select 1 from rental_agreement_items i
                  where i.agreement_id = a.id and i.item_status = 'on_rent')
    returning a.id, a.agreement_number, a.created_by, a.expected_return_date
  loop
    recipient := r.created_by;
    if recipient is null then
      select id into recipient from profiles where role = 'owner' and is_active limit 1;
    ...
    values (
      recipient,
      'Rental overdue: ' || r.agreement_number,
      'Expected back on ' || to_char(r.expected_return_date, 'DD Mon YYYY')
        || ' and one or more units are still out.',
      r.expected_return_date::timestamptz,
      'rental_agreement', r.id, recipient, 'high', 'pending'
    )
    returning id into act_id;
    ...
    values (recipient, 'task_assigned', null, act_id,
      'Rental overdue: ' || r.agreement_number,
      'Due back ' || to_char(r.expected_return_date, 'DD Mon YYYY'),
      '/dashboard/rentals/' || r.id);
  end loop;
end;
$$;
...
ALTER FUNCTION "public"."scan_rental_cycles"() OWNER TO "postgres";
...
begin
  ...
  set amount_paid = v_total,
      payment_status = case
        when v_sale_total is not null and v_sale_total <= 0 then 'paid'
        when v_total <= 0 then 'pending'
...
CREATE TABLE IF NOT EXISTS "public"."accessory_replacement_job_parts" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "accessory_replacement_job_id" "uuid" NOT NULL,
...
ALTER TABLE "public"."accessory_replacement_job_parts" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."accessory_replacement_jobs" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "job_number" "text",
    ...
    "job_date" "date",
    "sale_id" "uuid",
    CONSTRAINT "accessory_replacement_jobs_payment_account_check" CHECK (("payment_account" = ANY (ARRAY['Digitalbluez'::"text", 'Techtenth'::"text", 'Cash'::"text"]))),
    CONSTRAINT "accessory_replacement_jobs_status_check" CHECK (("status" = ANY (ARRAY['intake'::"text", 'in_progress'::"text", 'done'::"text", 'cancelled'::"text"])))
);


ALTER TABLE "public"."accessory_replacement_jobs" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."accessory_rma_events" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "sku_id" "uuid" NOT NULL,
    "quantity" integer NOT NULL,
    ...
    CONSTRAINT "accessory_rma_events_direction_check" CHECK (("direction" = ANY (ARRAY['to_vendor'::"text", 'from_customer'::"text"]))),
    CONSTRAINT "accessory_rma_events_quantity_check" CHECK (("quantity" > 0)),
    CONSTRAINT "accessory_rma_events_status_check" CHECK (("status" = ANY (ARRAY['initiated'::"text", 'shipped'::"text", 'vendor_accepted'::"text", 'vendor_rejected'::"text", 'replacement_received'::"text", 'refund_received'::"text", 'restocked'::"text", 'scrapped'::"text", 'closed'::"text"])))
);


ALTER TABLE "public"."accessory_rma_events" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."activities" (
    ...
    CONSTRAINT "activities_related_type_check" CHECK ((("related_type" IS NULL) OR ("related_type" = ANY (ARRAY['customer'::"text", 'sale'::"text", 'purchase_order'::"text", 'asset'::"text", 'repair_job'::"text", 'invoice'::"text", 'vendor'::"text", 'recurring_expense'::"text", 'marketing_asset'::"text", 'rental_agreement'::"text"])))),
...
CREATE TABLE IF NOT EXISTS "public"."asset_ledger" (
    ...
    CONSTRAINT "asset_ledger_status_check" CHECK (("status" = ANY (ARRAY['draft'::"text", 'reserved'::"text", 'received'::"text", 'in_stock'::"text", 'sold'::"text", 'faulty'::"text", 'returned'::"text", 'qc_pending'::"text", 'qc_passed'::"text", 'ready_for_sale'::"text", 'rma_sent'::"text", 'rma_returned'::"text", 'scrapped'::"text", 'pending_sale'::"text", 'pending_replacement'::"text", 'reserved_web'::"text", 'on_rent'::"text"]))),
...
CREATE TABLE IF NOT EXISTS "public"."bank_transaction_matches" (
    ...
    CONSTRAINT "bank_transaction_matches_match_type_check" CHECK (("match_type" = ANY (ARRAY['sale_payment'::"text", 'vendor_payment'::"text", 'expense'::"text", 'transfer_pair'::"text", 'stock_purchase'::"text"])))
);
...
COMMENT ON COLUMN "public"."business_profiles"."upi_id" IS 'UPI payment ID shown on invoices/quotations/proformas alongside the QR code.';



COMMENT ON COLUMN "public"."business_profiles"."qr_code_url" IS 'Storage key (purchase-files bucket) for this entity''s payment QR code image, shown on invoices/quotations/proformas.';
...
CREATE TABLE IF NOT EXISTS "public"."homepage_banners" (
    ...
    CONSTRAINT "homepage_banners_theme_check" CHECK (("theme" = ANY (ARRAY['default'::"text", 'diwali'::"text", 'christmas'::"text", 'sale'::"text", 'custom'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."invoice_items" (
    ...
    CONSTRAINT "invoice_items_item_type_check" CHECK (("item_type" = ANY (ARRAY['asset'::"text", 'accessory'::"text", 'custom'::"text", 'repair'::"text", 'rental'::"text"])))
);
...
COMMENT ON COLUMN "public"."market_price_observations"."warranty_months" IS 'Competitor-stated warranty period in months at the time the price was observed, for apples-to-apples comparison against this business''s own warranty terms.';
...
CREATE TABLE IF NOT EXISTS "public"."marketing_assets" (
    ...
    "format" "text" DEFAULT 'none'::"text" NOT NULL,
    "pillar" "text",
    "title" "text",
    "body_text" "text",
    ...
    "cta_text" "text",
    "alt_text" "text",
    "source_sku_ids" "uuid"[] DEFAULT '{}'::"uuid"[] NOT NULL,
    "source_filter" "jsonb",
    "card_paths" "text"[] DEFAULT '{}'::"text"[] NOT NULL,
    "status" "text" DEFAULT 'draft'::"text" NOT NULL,
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "marketing_assets_format_check" CHECK (("format" = ANY (ARRAY['wa_text'::"text", 'wa_square'::"text", 'ig_portrait'::"text", 'ig_story'::"text", 'fb_link'::"text", 'none'::"text"]))),
    CONSTRAINT "marketing_assets_kind_check" CHECK (("kind" = ANY (ARRAY['single_product'::"text", 'product_list'::"text", 'educational'::"text", 'offer'::"text", 'testimonial'::"text", 'blog'::"text", 'newsletter'::"text"]))),
    CONSTRAINT "marketing_assets_platform_check" CHECK (("platform" = ANY (ARRAY['whatsapp'::"text", 'instagram'::"text", 'facebook'::"text", 'google_business'::"text", 'blog'::"text", 'email'::"text"]))),
    CONSTRAINT "marketing_assets_status_check" CHECK (("status" = ANY (ARRAY['draft'::"text", 'approved'::"text", 'scheduled'::"text", 'published'::"text", 'archived'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."marketing_settings" (
    ...
    "default_cta" "text",
    "contact_block" "text",
    "hashtag_bank" "text"[] DEFAULT '{}'::"text"[] NOT NULL,
    "disclaimer" "text",
    "daily_generation_cap" integer DEFAULT 50 NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "whatsapp_flavor_lines" "text"[] DEFAULT '{}'::"text"[] NOT NULL,
    "default_warranty_label" "text",
    CONSTRAINT "marketing_settings_id_check" CHECK ("id")
);


ALTER TABLE "public"."marketing_settings" OWNER TO "postgres";


COMMENT ON TABLE "public"."marketing_settings" IS 'Singleton config for the marketing content generator (brand voice, default CTA/contact block, hashtag bank, daily AI-generation cap). Owner-only via Settings -> Marketing, same pattern as digest_channel_config.';
...
CREATE TABLE IF NOT EXISTS "public"."profile_page_actions" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "profile_page_actions_page_key_check" CHECK (("page_key" = ANY (ARRAY['new_entry'::"text", 'accessories'::"text", 'repair_jobs'::"text", 'replacement_jobs'::"text", 'sku_master'::"text", 'live_stock'::"text", 'invoices'::"text", 'customers'::"text", 'activities'::"text", 'sales'::"text", 'stock'::"text", 'website'::"text", 'expenses'::"text", 'quotations'::"text", 'rma'::"text", 'marketing'::"text", 'rentals'::"text"])))
);
...
COMMENT ON COLUMN "public"."sku_master"."warranty_label" IS 'Owner-set warranty tag shown/filterable on the storefront listing pages (e.g. "6 Months", "1 Year"). Independent of asset_ledger.warranty_duration_months, which tracks the actual warranty of a specific physical unit.';
...
ALTER VIEW "public"."public_banners" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."rental_agreement_counter" (
    "year" "text" NOT NULL,
    "last_number" integer DEFAULT 0 NOT NULL
);


ALTER TABLE "public"."rental_agreement_counter" OWNER TO "postgres";


CREATE TABLE IF NOT EXISTS "public"."rental_agreement_items" (
    "id" "uuid" DEFAULT "extensions"."uuid_generate_v4"() NOT NULL,
    "agreement_id" "uuid" NOT NULL,
    "asset_id" "uuid" NOT NULL,
    "item_status" "text" DEFAULT 'on_rent'::"text" NOT NULL,
    "handed_over_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "returned_at" timestamp with time zone,
    "return_condition_notes" "text",
    "buyout_sale_id" "uuid",
    "created_at" timestamp with time zone DEFAULT "now"(),
    CONSTRAINT "rental_agreement_items_item_status_check" CHECK (("item_status" = ANY (ARRAY['on_rent'::"text", 'returned'::"text", 'bought_out'::"text", 'lost_damaged'::"text"])))
);


ALTER TABLE "public"."rental_agreement_items" OWNER TO "postgres";


COMMENT ON TABLE "public"."rental_agreement_items" IS 'One physical serialized unit out on one rental. Scoped to serialized categories (LAP/DES/MON/TAB) -- fungible accessories have no asset_ledger row and are out of scope. This is where the unit<->rental link lives: a rent-charge `sales` row deliberately leaves asset_ledger_id NULL so the unit never reads as sold and its purchase cost is never charged as COGS against one month of rent.';
...
CREATE TABLE IF NOT EXISTS "public"."rental_agreements" (
    ...
    "billing_reminder_lead_days" integer DEFAULT 3 NOT NULL,
    ...
    "deposit_payment_account" "text",
    "deposit_received_at" timestamp with time zone,
    "deposit_refunded_amount" numeric(12,2),
    ...
    "deleted_at" timestamp with time zone,
    "deleted_remarks" "text",
    CONSTRAINT "rental_agreements_billing_interval_check" CHECK (("billing_interval" = ANY (ARRAY['one_time'::"text", 'monthly'::"text", 'quarterly'::"text"]))),
    CONSTRAINT "rental_agreements_deposit_payment_account_check" CHECK ((("deposit_payment_account" IS NULL) OR ("deposit_payment_account" = ANY (ARRAY['Digitalbluez'::"text", 'Techtenth'::"text", 'Cash'::"text"])))),
    CONSTRAINT "rental_agreements_payment_account_check" CHECK ((("payment_account" IS NULL) OR ("payment_account" = ANY (ARRAY['Digitalbluez'::"text", 'Techtenth'::"text", 'Cash'::"text"])))),
    CONSTRAINT "rental_agreements_status_check" CHECK (("status" = ANY (ARRAY['draft'::"text", 'active'::"text", 'closed'::"text", 'cancelled'::"text"])))
);


ALTER TABLE "public"."rental_agreements" OWNER TO "postgres";


COMMENT ON TABLE "public"."rental_agreements" IS 'One rental deal: a customer takes N units from ordinary sellable stock for a period. Rent is billed per billing_interval into real `sales` rows (sales.rental_agreement_id) -- there is no separate rental revenue ledger. Overdue is NEVER stored: it is derived from expected_return_date vs an item still on_rent. overdue_notified_at / billing_notified_at are cron dedup markers only, not status.';



COMMENT ON COLUMN "public"."rental_agreements"."security_deposit_amount" IS 'Refundable liability held against the agreement. Deliberately NOT a sales row -- a deposit is not revenue and must never attract GST or inflate reporting. A forfeited portion is recorded separately as a rental charge at closure, which is when it genuinely becomes income.';
 ...
 SELECT "s"."id",
    ...
    COALESCE("sku_a"."category", "sku_b"."category") AS "category",
        CASE
            ...
            WHEN ("s"."rental_agreement_id" IS NOT NULL) THEN 'rental'::"text"
            ELSE 'other'::"text"
        END AS "line_kind",
    "s"."asset_ledger_id",
    "a"."asset_number",
...
ALTER TABLE "public"."website_health_checks" OWNER TO "postgres";


ALTER TABLE "public"."website_health_checks" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."website_health_checks_id_seq"
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
ALTER TABLE ONLY "public"."accessory_replacement_job_parts"
    ADD CONSTRAINT "accessory_replacement_job_parts_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."accessory_replacement_jobs"
    ADD CONSTRAINT "accessory_replacement_jobs_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."accessory_rma_events"
    ADD CONSTRAINT "accessory_rma_events_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."festival_calendar"
    ADD CONSTRAINT "festival_calendar_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."rental_agreement_counter"
    ADD CONSTRAINT "rental_agreement_counter_pkey" PRIMARY KEY ("year");



ALTER TABLE ONLY "public"."rental_agreement_items"
    ADD CONSTRAINT "rental_agreement_items_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."rental_agreements"
    ADD CONSTRAINT "rental_agreements_agreement_number_key" UNIQUE ("agreement_number");
...
ALTER TABLE ONLY "public"."website_health_checks"
    ADD CONSTRAINT "website_health_checks_pkey" PRIMARY KEY ("id");
...
CREATE UNIQUE INDEX "customers_active_phone_unique" ON "public"."customers" USING "btree" ("btrim"("phone")) WHERE (("is_deleted" = false) AND ("phone" IS NOT NULL) AND ("btrim"("phone") <> ''::"text"));



CREATE INDEX "festival_calendar_date_idx" ON "public"."festival_calendar" USING "btree" ("festival_date");



CREATE INDEX "idx_accessory_replacement_job_parts_job" ON "public"."accessory_replacement_job_parts" USING "btree" ("accessory_replacement_job_id");



CREATE INDEX "idx_accessory_replacement_jobs_customer" ON "public"."accessory_replacement_jobs" USING "btree" ("customer_id");



CREATE INDEX "idx_accessory_replacement_jobs_status" ON "public"."accessory_replacement_jobs" USING "btree" ("status");



CREATE INDEX "idx_accessory_rma_events_created_by" ON "public"."accessory_rma_events" USING "btree" ("created_by");



CREATE INDEX "idx_accessory_rma_events_sku" ON "public"."accessory_rma_events" USING "btree" ("sku_id");



CREATE INDEX "idx_accessory_rma_events_status" ON "public"."accessory_rma_events" USING "btree" ("status");



CREATE INDEX "idx_accessory_rma_events_vendor_id" ON "public"."accessory_rma_events" USING "btree" ("vendor_id");
...
CREATE INDEX "idx_asset_ledger_asset_number_trgm" ON "public"."asset_ledger" USING "gin" ("asset_number" "extensions"."gin_trgm_ops");



CREATE INDEX "idx_asset_ledger_current_sku_id" ON "public"."asset_ledger" USING "btree" ("current_sku_id");
...
CREATE INDEX "idx_asset_ledger_serial_number_trgm" ON "public"."asset_ledger" USING "gin" ("serial_number" "extensions"."gin_trgm_ops");
...
CREATE INDEX "idx_marketing_assets_platform" ON "public"."marketing_assets" USING "btree" ("platform");



CREATE INDEX "idx_marketing_assets_scheduled_for" ON "public"."marketing_assets" USING "btree" ("scheduled_for");



CREATE INDEX "idx_marketing_assets_status" ON "public"."marketing_assets" USING "btree" ("status");
...
CREATE INDEX "idx_rental_agreement_items_agreement_id" ON "public"."rental_agreement_items" USING "btree" ("agreement_id");



CREATE INDEX "idx_rental_agreement_items_asset_id" ON "public"."rental_agreement_items" USING "btree" ("asset_id");
...
CREATE INDEX "idx_rental_agreements_expected_return_date" ON "public"."rental_agreements" USING "btree" ("expected_return_date");



CREATE INDEX "idx_rental_agreements_next_billing_date" ON "public"."rental_agreements" USING "btree" ("next_billing_date");



CREATE INDEX "idx_rental_agreements_status" ON "public"."rental_agreements" USING "btree" ("status");
...
CREATE INDEX "idx_repair_job_parts_sale_id" ON "public"."repair_job_parts" USING "btree" ("sale_id");
...
CREATE INDEX "idx_sales_customer_name_trgm" ON "public"."sales" USING "gin" ("customer_name" "extensions"."gin_trgm_ops");
...
CREATE INDEX "idx_sales_invoice_number_trgm" ON "public"."sales" USING "gin" ("invoice_number" "extensions"."gin_trgm_ops");



CREATE INDEX "idx_sales_rental_agreement_id" ON "public"."sales" USING "btree" ("rental_agreement_id");
...
CREATE INDEX "idx_sku_master_brand_trgm" ON "public"."sku_master" USING "gin" ("brand" "extensions"."gin_trgm_ops");
...
CREATE INDEX "idx_sku_master_full_sku_code_trgm" ON "public"."sku_master" USING "gin" ("full_sku_code" "extensions"."gin_trgm_ops");



CREATE INDEX "idx_sku_master_model_name_trgm" ON "public"."sku_master" USING "gin" ("model_name" "extensions"."gin_trgm_ops");



CREATE INDEX "idx_sku_master_sku_description_trgm" ON "public"."sku_master" USING "gin" ("sku_description" "extensions"."gin_trgm_ops");
...
CREATE INDEX "idx_user_sessions_profile_active" ON "public"."user_sessions" USING "btree" ("profile_id") WHERE ("revoked_at" IS NULL);



CREATE INDEX "idx_user_sessions_profile_last_seen" ON "public"."user_sessions" USING "btree" ("profile_id", "last_seen_at");
...
CREATE INDEX "idx_website_health_checks_checked_at" ON "public"."website_health_checks" USING "btree" ("checked_at" DESC);
...
CREATE UNIQUE INDEX "uq_rental_item_asset_on_rent" ON "public"."rental_agreement_items" USING "btree" ("asset_id") WHERE ("item_status" = 'on_rent'::"text");
...
ALTER TABLE ONLY "public"."accessory_replacement_job_parts"
    ADD CONSTRAINT "accessory_replacement_job_par_accessory_replacement_job_id_fkey" FOREIGN KEY ("accessory_replacement_job_id") REFERENCES "public"."accessory_replacement_jobs"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."accessory_replacement_job_parts"
    ADD CONSTRAINT "accessory_replacement_job_parts_sku_id_fkey" FOREIGN KEY ("sku_id") REFERENCES "public"."sku_master"("id");



ALTER TABLE ONLY "public"."accessory_replacement_job_parts"
    ADD CONSTRAINT "accessory_replacement_job_parts_stock_movement_id_fkey" FOREIGN KEY ("stock_movement_id") REFERENCES "public"."stock_movements"("id");



ALTER TABLE ONLY "public"."accessory_replacement_jobs"
    ADD CONSTRAINT "accessory_replacement_jobs_customer_id_fkey" FOREIGN KEY ("customer_id") REFERENCES "public"."customers"("id");



ALTER TABLE ONLY "public"."accessory_replacement_jobs"
    ADD CONSTRAINT "accessory_replacement_jobs_entered_by_fkey" FOREIGN KEY ("entered_by") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "public"."accessory_replacement_jobs"
    ADD CONSTRAINT "accessory_replacement_jobs_finalized_by_fkey" FOREIGN KEY ("finalized_by") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "public"."accessory_replacement_jobs"
    ADD CONSTRAINT "accessory_replacement_jobs_old_sku_id_fkey" FOREIGN KEY ("old_sku_id") REFERENCES "public"."sku_master"("id");



ALTER TABLE ONLY "public"."accessory_replacement_jobs"
    ADD CONSTRAINT "accessory_replacement_jobs_replacement_sku_id_fkey" FOREIGN KEY ("replacement_sku_id") REFERENCES "public"."sku_master"("id");



ALTER TABLE ONLY "public"."accessory_replacement_jobs"
    ADD CONSTRAINT "accessory_replacement_jobs_sale_id_fkey" FOREIGN KEY ("sale_id") REFERENCES "public"."sales"("id");



ALTER TABLE ONLY "public"."accessory_rma_events"
    ADD CONSTRAINT "accessory_rma_events_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "public"."accessory_rma_events"
    ADD CONSTRAINT "accessory_rma_events_inbound_movement_id_fkey" FOREIGN KEY ("inbound_movement_id") REFERENCES "public"."stock_movements"("id");



ALTER TABLE ONLY "public"."accessory_rma_events"
    ADD CONSTRAINT "accessory_rma_events_outbound_movement_id_fkey" FOREIGN KEY ("outbound_movement_id") REFERENCES "public"."stock_movements"("id");



ALTER TABLE ONLY "public"."accessory_rma_events"
    ADD CONSTRAINT "accessory_rma_events_sku_id_fkey" FOREIGN KEY ("sku_id") REFERENCES "public"."sku_master"("id");



ALTER TABLE ONLY "public"."accessory_rma_events"
    ADD CONSTRAINT "accessory_rma_events_vendor_id_fkey" FOREIGN KEY ("vendor_id") REFERENCES "public"."vendors"("id");
...
ALTER TABLE ONLY "public"."asset_ledger"
    ADD CONSTRAINT "asset_ledger_current_sku_id_fkey" FOREIGN KEY ("current_sku_id") REFERENCES "public"."sku_master"("id");
...
ALTER TABLE ONLY "public"."bank_transaction_matches"
    ADD CONSTRAINT "bank_transaction_matches_stock_movement_id_fkey" FOREIGN KEY ("stock_movement_id") REFERENCES "public"."stock_movements"("id");
...
ALTER TABLE ONLY "public"."homepage_banners"
    ADD CONSTRAINT "homepage_banners_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id");
...
ALTER TABLE ONLY "public"."marketing_assets"
    ADD CONSTRAINT "marketing_assets_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id");
...
ALTER TABLE ONLY "public"."rental_agreement_items"
    ADD CONSTRAINT "rental_agreement_items_agreement_id_fkey" FOREIGN KEY ("agreement_id") REFERENCES "public"."rental_agreements"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."rental_agreement_items"
    ADD CONSTRAINT "rental_agreement_items_asset_id_fkey" FOREIGN KEY ("asset_id") REFERENCES "public"."asset_ledger"("id");



ALTER TABLE ONLY "public"."rental_agreement_items"
    ADD CONSTRAINT "rental_agreement_items_buyout_sale_id_fkey" FOREIGN KEY ("buyout_sale_id") REFERENCES "public"."sales"("id");



ALTER TABLE ONLY "public"."rental_agreements"
    ADD CONSTRAINT "rental_agreements_customer_id_fkey" FOREIGN KEY ("customer_id") REFERENCES "public"."customers"("id");
...
ALTER TABLE ONLY "public"."repair_job_parts"
    ADD CONSTRAINT "repair_job_parts_sale_id_fkey" FOREIGN KEY ("sale_id") REFERENCES "public"."sales"("id");
...
ALTER TABLE ONLY "public"."sales"
    ADD CONSTRAINT "sales_rental_agreement_id_fkey" FOREIGN KEY ("rental_agreement_id") REFERENCES "public"."rental_agreements"("id");
...
CREATE POLICY "Allow full access to auth users" ON "public"."accessory_rma_events" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "Owner manages homepage banners" ON "public"."homepage_banners" USING (( SELECT "public"."is_owner"() AS "is_owner")) WITH CHECK (( SELECT "public"."is_owner"() AS "is_owner"));
...
CREATE POLICY "Staff read health checks" ON "public"."website_health_checks" FOR SELECT USING (( SELECT "public"."is_staff"() AS "is_staff"));
...
ALTER TABLE "public"."accessory_replacement_job_parts" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "accessory_replacement_job_parts_authenticated" ON "public"."accessory_replacement_job_parts" USING ((( SELECT "auth"."role"() AS "role") = 'authenticated'::"text")) WITH CHECK ((( SELECT "auth"."role"() AS "role") = 'authenticated'::"text"));



ALTER TABLE "public"."accessory_replacement_jobs" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "accessory_replacement_jobs_authenticated" ON "public"."accessory_replacement_jobs" USING ((( SELECT "auth"."role"() AS "role") = 'authenticated'::"text")) WITH CHECK ((( SELECT "auth"."role"() AS "role") = 'authenticated'::"text"));



ALTER TABLE "public"."accessory_rma_events" ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE "public"."rental_agreement_counter" ENABLE ROW LEVEL SECURITY;


CREATE POLICY "rental_agreement_counter_staff" ON "public"."rental_agreement_counter" TO "authenticated" USING (( SELECT "public"."is_staff"() AS "is_staff")) WITH CHECK (( SELECT "public"."is_staff"() AS "is_staff"));
...
CREATE POLICY "rental_agreement_items_staff" ON "public"."rental_agreement_items" TO "authenticated" USING (( SELECT "public"."is_staff"() AS "is_staff")) WITH CHECK (( SELECT "public"."is_staff"() AS "is_staff"));
...
CREATE POLICY "rental_agreements_staff" ON "public"."rental_agreements" TO "authenticated" USING (( SELECT "public"."is_staff"() AS "is_staff")) WITH CHECK (( SELECT "public"."is_staff"() AS "is_staff"));
...
GRANT ALL ON FUNCTION "public"."generate_rental_agreement_number"() TO "anon";
GRANT ALL ON FUNCTION "public"."generate_rental_agreement_number"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."generate_rental_agreement_number"() TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."report_category_entity_matrix"("p_from" "date", "p_to" "date", "p_source" "text", "p_include_financials" boolean) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."report_category_entity_matrix"("p_from" "date", "p_to" "date", "p_source" "text", "p_include_financials" boolean) TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."report_purchase_kpis"("p_from" "date", "p_to" "date") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."report_purchase_kpis"("p_from" "date", "p_to" "date") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."report_rentals"("p_from" "date", "p_to" "date") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."report_rentals"("p_from" "date", "p_to" "date") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."report_web_funnel"("p_from" "date", "p_to" "date", "p_abandon_hours" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."report_web_funnel"("p_from" "date", "p_to" "date", "p_abandon_hours" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."report_web_funnel"("p_from" "date", "p_to" "date", "p_abandon_hours" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."report_web_funnel"("p_from" "date", "p_to" "date", "p_abandon_hours" integer) TO "service_role";



REVOKE ALL ON FUNCTION "public"."report_web_funnel_timeseries"("p_from" "date", "p_to" "date") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."report_web_funnel_timeseries"("p_from" "date", "p_to" "date") TO "anon";
GRANT ALL ON FUNCTION "public"."report_web_funnel_timeseries"("p_from" "date", "p_to" "date") TO "authenticated";
GRANT ALL ON FUNCTION "public"."report_web_funnel_timeseries"("p_from" "date", "p_to" "date") TO "service_role";



REVOKE ALL ON FUNCTION "public"."report_website_health"("p_from" timestamp with time zone, "p_to" timestamp with time zone) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."report_website_health"("p_from" timestamp with time zone, "p_to" timestamp with time zone) TO "anon";
GRANT ALL ON FUNCTION "public"."report_website_health"("p_from" timestamp with time zone, "p_to" timestamp with time zone) TO "authenticated";
GRANT ALL ON FUNCTION "public"."report_website_health"("p_from" timestamp with time zone, "p_to" timestamp with time zone) TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."run_website_health_check"("p_url" "text") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."run_website_health_check"("p_url" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."run_website_health_check"("p_url" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."run_website_health_check"("p_url" "text") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."scan_rental_cycles"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."scan_rental_cycles"() TO "service_role";
...
GRANT ALL ON TABLE "public"."accessory_replacement_job_parts" TO "anon";
GRANT ALL ON TABLE "public"."accessory_replacement_job_parts" TO "authenticated";
GRANT ALL ON TABLE "public"."accessory_replacement_job_parts" TO "service_role";



GRANT ALL ON TABLE "public"."accessory_replacement_jobs" TO "anon";
GRANT ALL ON TABLE "public"."accessory_replacement_jobs" TO "authenticated";
GRANT ALL ON TABLE "public"."accessory_replacement_jobs" TO "service_role";



GRANT ALL ON TABLE "public"."accessory_rma_events" TO "anon";
GRANT ALL ON TABLE "public"."accessory_rma_events" TO "authenticated";
GRANT ALL ON TABLE "public"."accessory_rma_events" TO "service_role";
...
GRANT ALL ON TABLE "public"."festival_calendar" TO "anon";
GRANT ALL ON TABLE "public"."festival_calendar" TO "authenticated";
GRANT ALL ON TABLE "public"."festival_calendar" TO "service_role";
...
GRANT ALL ON TABLE "public"."marketing_settings" TO "anon";
GRANT ALL ON TABLE "public"."marketing_settings" TO "authenticated";
GRANT ALL ON TABLE "public"."marketing_settings" TO "service_role";
...
GRANT ALL ON TABLE "public"."public_banners" TO "anon";
GRANT ALL ON TABLE "public"."public_banners" TO "authenticated";
GRANT ALL ON TABLE "public"."public_banners" TO "service_role";
...
GRANT ALL ON TABLE "public"."rental_agreement_counter" TO "anon";
GRANT ALL ON TABLE "public"."rental_agreement_counter" TO "authenticated";
GRANT ALL ON TABLE "public"."rental_agreement_counter" TO "service_role";



GRANT ALL ON TABLE "public"."rental_agreement_items" TO "anon";
GRANT ALL ON TABLE "public"."rental_agreement_items" TO "authenticated";
GRANT ALL ON TABLE "public"."rental_agreement_items" TO "service_role";



GRANT ALL ON TABLE "public"."rental_agreements" TO "anon";
GRANT ALL ON TABLE "public"."rental_agreements" TO "authenticated";
...
GRANT ALL ON TABLE "public"."website_health_checks" TO "anon";
GRANT ALL ON TABLE "public"."website_health_checks" TO "authenticated";
GRANT ALL ON TABLE "public"."website_health_checks" TO "service_role";



GRANT ALL ON SEQUENCE "public"."website_health_checks_id_seq" TO "anon";
GRANT ALL ON SEQUENCE "public"."website_health_checks_id_seq" TO "authenticated";
GRANT ALL ON SEQUENCE "public"."website_health_checks_id_seq" TO "service_role";
