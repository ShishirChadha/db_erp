...
Live schema snapshot -- 79 tables. Regenerate with `npm run bible:generate`.
...
- [users](#users)
- [vendor_correction_proposals](#vendorcorrectionproposals)
...
| text_layer_chars | integer | yes |  |  |  |
| extraction_tier | text | yes |  |  |  |
| extraction_status | text | no | `'pending'::text` |  |  |
| extraction_template_id | uuid | yes |  | FK -> extraction_templates.id |  |
| vendor_id | uuid | yes |  | FK -> vendors.id |  |
| bank_account_id | uuid | yes |  | FK -> bank_accounts.id |  |
| document_date | date | yes |  |  |  |
| ai_approved_by | uuid | yes |  | FK -> users.id |  |
| ai_approved_at | timestamp with time zone | yes |  |  |  |
...
|---|---|---|---|---|---|
| id | uuid | no | `gen_random_uuid()` | PK |  |
| document_id | uuid | no |  | FK -> uploaded_documents.id |  |
| vendor_id | uuid | no |  | FK -> vendors.id |  |
| field_name | text | no |  |  |  |
| current_value | text | yes |  |  |  |
| proposed_value | text | yes |  |  |  |
| change_kind | text | no |  |  |  |
| confidence | text | no | `'high'::text` |  |  |
| status | text | no | `'pending'::text` |  |  |
| decided_by | uuid | yes |  | FK -> users.id |  |
| decided_at | timestamp with time zone | yes |  |  |  |
| field_correction_id | uuid | yes |  |  |  |
| created_at | timestamp with time zone | no | `now()` |  |  |
