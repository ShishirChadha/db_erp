...
keywords: [purchase, po, purchase order, kharidna, vendor bill, purchase invoice, stock intake, reserve assets]
sources:
...
- **Attaching units to an existing PO** (`from-intake`, `from-accessory-stock`)
  is how units that entered via employee intake or accessory receipt get their
  formal paperwork retroactively — see **attach-units-to-po**.
- **`purchase_invoices`** — the vendor's actual bill, which can span multiple
  POs. This is where GST input credit tracking lives.

## Related

**receive-stock**, **attach-units-to-po**, **finance-gst-reports** (for GST
input-side treatment), **business-rules** (asset numbering is a PO artifact,
never an inventory-existence requirement).
